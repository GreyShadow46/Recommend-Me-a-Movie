  ...
  con.connect(function(err) {
    ...
    con.query("SELECT * FROM accounts WHERE userName = '" + req.body.username + "'", function (err, result, fields) {
      if(result.length !== 0){
        ...
        sendMail(req.body.username, req.body.email, "Verify your Email", 'Please press this <a href="http://ec2-13-41-200-144.eu-west-2.compute.amazonaws.com/surveypage1">button</a> to verify your account');
  ...
  con.connect(function(err) {
    ...
    con.query("SELECT * FROM accounts WHERE userName = '" + req.body.username + "' AND emailAddress = '" + req.body.email + "'", function (err, result, fields) {
      if(result.length !== 0){
        ...
        sendMail(username, req.body.email, "Reset Your Password", 'Please press this <a href="http://ec2-13-41-200-144.eu-west-2.compute.amazonaws.com/resetyourpassword">button</a> to proceed');
        res.redirect('/verifyemail');
      }
      else {
